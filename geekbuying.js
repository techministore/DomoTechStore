/**
 * DomoTechStore - Geekbuying Worker
 * ✔ Proxy seguro para la API de afiliados de Geekbuying
 * ✔ Firma MD5 compatible con Cloudflare Workers
...
function generateGeekbuyingSign(params, appSecret) {
    const sortedKeys = Object.keys(params).sort();
    const queryString = sortedKeys.map(k => `${k}=${params[k]}`).join('&');
    const fullString = `${queryString}${appSecret}`;
...
async function searchProducts(appId, appSecret, keyword, page = 1) {
    ...
    params.sign = generateGeekbuyingSign(params, appSecret);

    const url = 'https://apiservice.geekbuying.com/api/product/search?' + new URLSearchParams(params);
    ...
    if (!res.ok) {
        console.error('[GEEKBUYING] HTTP error:', res.status);
        return null;
    }
...
// ============================================================================
// 🏷️ OFERTAS / PRODUCTOS POPULARES
// ============================================================================

async function getDeals(appId, appSecret) {
    const timestamp = Math.floor(Date.now() / 1000).toString();
    const params = {
        app_id: appId,
        page: '1',
    ...
    const url = 'https://apiservice.geekbuying.com/api/product/hot?' + new URLSearchParams(params);
...
export async function onRequest(context) {
    ...
    const APP_ID = env.GEEKBUYING_APP_ID;
    const APP_SECRET = env.GEEKBUYING_APP_SECRET;
    ...
    if (!APP_ID || !APP_SECRET) {
        console.warn('[GEEKBUYING] Credenciales no configuradas, usando fallback');
    ...
    try {
        if (action === 'search') {
            const data = await searchProducts(APP_ID, APP_SECRET, q);
        ...
        if (action === 'deals') {
            const data = await getDeals(APP_ID, APP_SECRET);
        ...
        console.error('[GEEKBUYING] Error:', err);
        return new Response(JSON.stringify({
            code: -1,
            error: err.message,
            fallback: true
