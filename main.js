...
const STORE_SETTINGS = {
    ...
    GEEKBUYING_ENABLED: true,
    TEMU_ENABLED: true,
    EBAY_ENABLED: true,
    MIRAVIA_ENABLED: true,
    CAFAGO_ENABLED: true,
    TOMTOP_ENABLED: true
};

const STORE_CONFIG = {
    ALIEXPRESS: { name: 'AliExpress', color: '#ff4747', prefix: 'ali_' },
    BANGGOOD:   { name: 'Banggood',   color: '#2196F3', prefix: 'bg_'  },
    GEEKBUYING: { name: 'Geekbuying', color: '#ff6b2b', prefix: 'gb_'  },
    TEMU:       { name: 'Temu',       color: '#ff6900', prefix: 'tm_'  },
    EBAY:       { name: 'eBay',       color: '#e53238', prefix: 'eb_'  },
    MIRAVIA:    { name: 'Miravia',    color: '#0c2340', prefix: 'mir_' },
    CAFAGO:     { name: 'Cafago',     color: '#e8321e', prefix: 'caf_' },
    TOMTOP:     { name: 'TomTop',     color: '#f26522', prefix: 'tt_'  }
};

// Configuración de Banggood (ahora usando Worker de Cloudflare)
const BANGGOOD_CONFIG = {
    AFFILIATE_ID: '' // Añade tu ID de afiliado aquí si lo tienes
};

// Configuración de tiendas sin API propia (IDs de afiliado)
window.GEEKBUYING_CONFIG = { AFFILIATE_ID: '' };
window.TEMU_CONFIG       = { AFFILIATE_ID: '' };
window.EBAY_CAMPAIGN_ID  = ''; // eBay Partner Network Campaign ID
window.MIRAVIA_AWIN_MID  = '30521';
window.MIRAVIA_AWIN_AFFID = '1636287'; // Tu Awin Publisher ID
window.CAFAGO_AFFILIATE_ID = '';
window.TOMTOP_AFFILIATE_ID = '';
...
async function searchBanggood(keyword) {
    ...
    try {
        ...
        console.log("[BANGGOOD] Worker response:", data);
        ...
        if (!data || data.code !== 0) {
            ...
            const normalized = products.map(p => normalizeProduct(p, 'BANGGOOD')).filter(p => p !== null);
            if (normalized.length > 0) {
                return normalized;
            }
        }
    } catch (err) {
        console.warn("[BANGGOOD] Error en la solicitud:", err);
    }

    // Fallback: productos de demostración
    console.warn("[BANGGOOD] Usando productos de demostración");
    const demoProducts = [
        {
            product_id: '1234567',
...
// ============================================================================
// 🔗 MÓDULO 2: MOTOR DE FUSIÓN ALIEXPRESS + BANGGOOD + GEEKBUYING
// ============================================================================
async function fusedSearch(keyword, options = {}) {
    const { 
        prioritize = 'bg-first', // 'best-price', 'ali-first', 'bg-first', 'gb-first'
        maxItems = 20,
        useFallback = true
    } = options;
    ...
    if (STORE_SETTINGS.ALIEXPRESS_ENABLED) {
        searchTasks.push({
            store: 'ALIEXPRESS',
            promise: fetchWithRetry(keyword, true).then(p => (p || []).map(prod => normalizeProduct({...prod, id: prod.id}, 'ALIEXPRESS')))
        });
    }
    ...
    if (STORE_SETTINGS.BANGGOOD_ENABLED) {
        searchTasks.push({
            store: 'BANGGOOD',
            promise: searchBanggood(keyword)
        });
    }

    if (STORE_SETTINGS.GEEKBUYING_ENABLED) {
        searchTasks.push({
            store: 'GEEKBUYING',
            promise: (typeof window.searchGeekbuying === 'function')
                ? window.searchGeekbuying(keyword)
                : Promise.resolve([])
        });
    }

    if (STORE_SETTINGS.TEMU_ENABLED) {
        searchTasks.push({
            store: 'TEMU',
            promise: (typeof window.searchTemu === 'function')
                ? window.searchTemu(keyword)
                : Promise.resolve([])
        });
    }

    if (STORE_SETTINGS.EBAY_ENABLED) {
        searchTasks.push({
            store: 'EBAY',
            promise: (typeof window.searchEbay === 'function')
                ? window.searchEbay(keyword)
                : Promise.resolve([])
        });
    }

    if (STORE_SETTINGS.MIRAVIA_ENABLED) {
        searchTasks.push({
            store: 'MIRAVIA',
            promise: (typeof window.searchMiravia === 'function')
                ? window.searchMiravia(keyword)
                : Promise.resolve([])
        });
    }

    if (STORE_SETTINGS.CAFAGO_ENABLED) {
        searchTasks.push({
            store: 'CAFAGO',
            promise: (typeof window.searchCafago === 'function')
                ? window.searchCafago(keyword)
                : Promise.resolve([])
        });
    }

    if (STORE_SETTINGS.TOMTOP_ENABLED) {
        searchTasks.push({
            store: 'TOMTOP',
            promise: (typeof window.searchTomtop === 'function')
                ? window.searchTomtop(keyword)
                : Promise.resolve([])
        });
    }
    ...
    switch (prioritize) {
        ...
        case 'ali-first':
            ...
            break;
        case 'gb-first':
            fusedResults.sort((a, b) => (a.store === 'GEEKBUYING' ? -1 : 1));
            break;
        case 'bg-first':
        default:
    ...
    Logger.info(`[FUSION] Resultados: ${allResults.length} productos totales (BG + GB + Ali)`);
